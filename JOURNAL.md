...
## 2026-07-06 (soir) — Carto-CEE v0 : cartographier les logements candidats aux aides
- **Cadrage** : *quand un délégataire ou un artisan RGE prospecte une zone, il veut voir quels logements sont candidats à quelles fiches CEE (et dispositifs cumulables), avec les signaux d'adaptation du lieu, afin de cibler ses visites.* Doctrine actée : candidature probable ≠ éligibilité (l'éligibilité CEE est par opération, pas par logement).
- **Fait** : `dossiers/04-carto-cee-cadrage.md` (architecture, moteur de règles, limites dont interdiction du démarchage téléphonique et biais « base DPE ≠ parc », 4 modèles économiques, trajectoire v1 BDNB / v2 croisement prix-risque) + `dossiers/carto-cee.html` : prototype autonome, recherche commune (geo.api.gouv.fr) → DPE géolocalisés (API ADEME data-fair, bbox) → clic logement → fiches candidates (JSON `REGLES` éditable : BAR-EN-101/102/103/104, BAR-TH-171/112, MPR ampleur, alertes locatives F/G, précarité déclarative) + signaux Géorisques (RGA, inondation). Repli intégral : mode démo (données fictives) et rendu points SVG si Leaflet/tuiles indisponibles. Bloc CONFIG unique pour les endpoints/champs.
- **DoD** : testé Playwright hors ligne : mode démo 9 points, cas « maison G fioul 1931 » → 8 fiches attendues, 2 signaux d'adaptation, filtres OK (E seul → 3 points), zéro erreur console, avertissements visibles. **Non testé ici** (réseau sortant bloqué par la politique de l'environnement) : le chemin live Leaflet + APIs ADEME/Géorisques → à vérifier par Johann au premier lancement dans un vrai navigateur ; endpoints et noms de champs regroupés dans CONFIG pour correction rapide.
- **Prochain** : lancer l'outil en ligne sur une vraie commune ; faire relire le JSON `REGLES` par un pro des CEE (terrain Énergie Responsable) ; v1 = agrégation BDNB par quartier.

## 2026-07-06 (suite) — Protocole préenregistré + script de données + maquette radar (décisions validées)
- **Fait** : décisions Johann actées (nomenclature P1-P7 validée avec P7 en anneau ; pilote = urbain ICU 75/92/93/94 ; ordre acteurs : oncle → Massiet du Biest → courtiers/CCI). Trois livrables : `dossiers/03-protocole-preenregistre.md` (H1-H4 figées, S1-S3, placebos, registre des écarts, commit = horodatage), `dossiers/scripts/assemble_donnees.py` (DVF téléchargé auto, BDNB/LCZ/PPRI/RGA en dépôts locaux guidés, filtres du protocole §5 implémentés, rapport de qualité), `dossiers/maquette-radar-boussole.html` (autonome, radar 6 rayons + anneau P7 à 3 états, panneau détail, curseurs démo, vue table, clair/sombre).
- **Pourquoi** : figer le protocole AVANT d'ouvrir les données (Claude Science ne pourra pas pêcher des résultats) ; la maquette porte la décision d'architecture « P7 = garde-fou, pas un score » face aux acteurs.
- **DoD** : `py_compile` OK sur le script ; palette du radar validée par le validateur dataviz (daltonisme + contraste, clair `#c2552f`/`#1f80b3`, sombre `#cf6238`/`#3d8db8`) ; rendu vérifié dans Chromium headless en 390 px et 1100 px, clair et sombre ; interactions testées via Playwright (clic P3 → panneau, P7 invalidé → anneau vermillon + polygone estompé) ; `scrollWidth` = 390 (zéro débordement, la table défile dans son conteneur). Données de la maquette explicitement fictives.
- **Prochain** : geler ce commit comme horodatage du protocole ; Johann dépose BDNB/LCZ/PPRI dans `dossiers/scripts/data/raw/` et lance l'étape `dvf` ; visio oncle avec la maquette.
